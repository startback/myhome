...
class IndexController extends CommonController {
    ...
    public function pass(){
        if($_POST){
            ...
            if(strtolower(md5($pass)) == strtolower(bin2hex($_SESSION['admin']['info']['admin_pass']))){
                $data['admin_pass'] = hex2bin(md5($new_pass));
                if(M('admin')->where('admin_id='.$_SESSION['admin']['info']['admin_id'])->save($data)){
                    $_SESSION['admin']['info']['admin_pass'] = $data['admin_pass'];
                    $this->success('修改成功');
                }else{
                    $this->error('修改失败');
                }
