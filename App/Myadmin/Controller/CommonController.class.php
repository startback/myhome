...
class CommonController extends Controller {
    ...
    public function __construct(){
		...
		//获取后台显表
		if(empty($_SESSION['admin']['info']['web_show'])){
			if($_SESSION['admin']['info']){
				$all_show = C('ADMIN_LEVEL_MENU');
				$cur_levels = $this->get_person_level($_SESSION['admin']['info']['admin_role_id']);
				if($_SESSION['admin']['info']['admin_account'] == 'admin'){
					$_SESSION['admin']['info']['web_show'] = $all_show;
				}else{
					$_SESSION['admin']['info']['web_show'] = $this->get_the_show_arr($all_show,$cur_levels);
				}
			}
		}
		$this->assign('admin_menu_show',$_SESSION['admin']['info']['web_show']);		
