...
class CommonController extends Controller {
    ...
    public function __construct(){
		...
		//获取后台显表
		if(empty($_SESSION['ttfadmin']['info']['web_show'])){
			if($_SESSION['ttfadmin']['info']){
				$all_show = C('ADMIN_LEVEL_MENU');
				$cur_levels = $this->get_person_level($_SESSION['ttfadmin']['info']['admin_role_id']);
				if($_SESSION['ttfadmin']['info']['admin_account'] == 'admin'){
					$_SESSION['ttfadmin']['info']['web_show'] = $all_show;
				}else{
					$_SESSION['ttfadmin']['info']['web_show'] = $this->get_the_show_arr($all_show,$cur_levels);
				}
			}
		}
		$this->assign('admin_menu_show',$_SESSION['ttfadmin']['info']['web_show']);

		
        //个人权限
        if(!in_array($path_info,C('NO_LEVEL'))) {
            if ($_SESSION['ttfadmin']['info']['admin_account'] != 'admin') {
                $user_levels = $this->get_person_level($_SESSION['ttfadmin']['info']['admin_role_id']);
    ...
    public function no_login($path_info){
        if(!in_array($path_info,C('NO_LOGIN'))){
            if(empty($_SESSION['ttfadmin']['info'])) {
    ...
    public function get_person_level($role_id){
        $roles = M('admin_role')->where('role_id='.$role_id)->find();
