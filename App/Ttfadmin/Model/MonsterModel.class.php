...
class MonsterModel extends Model {
    ...
    //获得页数 选择页面
    public function select_monster_page($page,$where){		
        $total_num = M('monster')->where($where)->count();
        $total_page = ceil($total_num/$this->per_page);
        $cur_page = $page['page'];
        $pre_page = $cur_page - 1;
        ...
        $base_purl = __ROOT__.'/index.php?m=ttfadmin&c=select&a=select_monster&monster_name='.$page['monster_name'].'&start_time='.$page['start_time'].'&end_time='.$page['end_time'].'&search_id='.$page['search_id'].'&search_id_2='.$page['search_id_2'].'&search_id_3='.$page['search_id_3'];
        $purl = $base_purl.'&p=';
